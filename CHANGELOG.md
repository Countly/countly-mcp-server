...
- **`drill_users_list` failed on every call** — `/o?method=segmentation_users` rejects a request without `bucket` (`HTTP 400: Missing request parameter: bucket`), even though the endpoint returns a flat uid list rather than a bucketed series. The handler now sends `bucket`, defaulting to `daily`, and exposes it as an optional parameter. Found by running the tool against a live Countly 26.x instance; the mocked unit tests passed while it was broken.
- **An unknown projection key looked like a successful breakdown** — Countly does not error on a key no event carries; it groups every matching event into a single `(Not set)` segment and returns correct totals, so the response is structurally indistinguishable from a real one-value breakdown. The advisory now recognizes the placeholder and says so explicitly, while a `(Not set)` bucket sitting alongside real values is left alone.
- **The empty-breakdown advisory fired on successful breakdowns** — it looked only at `meta`, but current Countly builds return `meta` as an empty **array** on a successful breakdown and put the per-value data in `segments` and inline under `data.<bucket>.<period>.<value>.keys`. A working `["did"]` breakdown was therefore annotated as having returned nothing. Detection now covers all three shapes, and `[]` is correctly read as empty rather than as populated. Also found only by the live run; the regression tests now use payloads copied from real responses.
...
### Added
- **`drill_users_list` tool** — wraps `/o?method=segmentation_users` and returns the Countly internal user ids (uid) matching a drill segmentation query. Takes an optional `bucket` (default `daily`), which Countly requires on the endpoint. This is the supported route for per-user analysis of an event, and it scales where a `projection_key` breakdown on the high-cardinality `did` field does not. Read-only, in the `drill` category, so it is gated on the drill plugin and available under `COUNTLY_TOOLS_DRILL=R`.
